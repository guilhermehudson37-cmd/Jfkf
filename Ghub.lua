--[[
    Ghub - Blox Fruits Script (Versão Melhorada - Detecção de Nível Aprimorada)
    Funcionalidades:
...
-- DETECÇÃO DE NÍVEL MELHORADA
local function getPlayerLevel()
    -- Tenta encontrar o level em vários lugares comuns
    local stats = player:FindFirstChild("leaderstats")
    if stats then
        -- Verifica nomes comuns: "Level", "Lvl", "Levels", "level"
        local levelObj = stats:FindFirstChild("Level") or stats:FindFirstChild("level") 
                      or stats:FindFirstChild("Lvl") or stats:FindFirstChild("Levels")
        if levelObj and typeof(levelObj.Value) == "number" then
            return levelObj.Value
        end
    end
    
    -- Tenta em ReplicatedStorage ou em outros lugares (alguns jogos usam)
    local levelData = player:FindFirstChild("Level") or player:FindFirstChild("level") 
                   or player:FindFirstChild("PlayerLevel")
    if levelData and typeof(levelData.Value) == "number" then
        return levelData.Value
    end
    
    -- Tenta em "Stats" se existir
    local playerStats = player:FindFirstChild("Stats")
    if playerStats then
        local lvl = playerStats:FindFirstChild("Level") or playerStats:FindFirstChild("level")
        if lvl and typeof(lvl.Value) == "number" then
            return lvl.Value
        end
    end
    
    -- Se não encontrar, assume nível 1 (evita travamento)
    warn("Nível não detectado! Usando nível 1 como fallback.")
    return 1
end

-- Encontra a missão adequada para o nível atual
local function getQuestForLevel(level)
    -- Se level for 0 ou nil, define como 1
    ...
    -- Se não encontrar, pega a última missão disponível (máximo)
    return quests[#quests]
end

-- Encontra um modelo pelo nome (parcial) com mais robustez
local function findModelByName(partialName)
    local found = nil
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and obj.Name:find(partialName) then
            -- Verifica se tem HumanoidRootPart
...
-- Encontra mobs da missão atual com tolerância a nomes
local function getQuestMobs()
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and obj ~= character then
            -- Verifica se o nome contém o nome do mob (case insensitive)
...
local function tweenToPosition(targetPos)
    -- Mantém a altura do player, mas se for muito diferente, ajusta
    local y = targetPos.Y
    if math.abs(y - rootPart.Position.Y) > 50 then
        y = rootPart.Position.Y -- mantém a altura atual se a diferença for grande
    end
    local pos = Vector3.new(targetPos.X, y, targetPos.Z)
    local tweenInfo = TweenInfo.new(1.0, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
...
local function interactWithNPC(npcModel)
    ...
    -- Muitas vezes o botão está mais em baixo, tenta várias posições
    for i = 1, 3 do
        local yOffset = 60 + (i-1) * 30
        local yPos = screenSize.Y / 2 + yOffset
...
local function takeQuest(quest)
    ...
    -- Tenta encontrar o NPC
    local npc = findModelByName(quest.npcName)
    if not npc then
        -- Tenta buscar por nome parcial (primeira palavra)
        local parts = string.split(quest.npcName, " ")
        if parts and #parts > 0 then
            npc = findModelByName(parts[1])
        end
        if not npc then
...
local function farmLoop()
    while isFarming and task.wait(0.1) do
        ...
        if not currentQuest then
            ...
            statusMsg = "Nível detectado: " .. tostring(level)
